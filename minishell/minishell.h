...
/*   Updated: 2023/09/26 19:32:44 by fgolino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start(void); //funzione che splitta l'intera riga letta da readline e la prepara all'analisi
void	free_matrix(char **matrix); //funzione che libera la memoria di una matrice
void	free_stuff(void); //funzione che libera tutta la memoria occupata dalla struttura globale
char	**pipe_finder(void); //funzione che trova la posizione di una pipe
...
void	analizer(void); //funzione che capisce se la nuova riga presenta un comando valido e lo fa partire
void	pwd_handler(void); //funzione che imita il funzionamento di pwd
void	echo_handler(void); //funzione che imita il funzionamente di echo (con flag -n)
void	cd_handler(void); //funzione che imita il comportamento di cd
int		redirector(void); //funzione che modifica momentaneamente il stdout
void	get_environment(void); //funzione che salva le variabili globali che ci servono e inizializza le variabili della struttura
int		check_string(char *str); //funzione che controlla se le quote della stringa siano correttamente chiuse
void	polish_tokens(void); //funzione che controlla la presenza di quotes nelle stringhe e fa partire quote_remover
